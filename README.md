...
- `Groq` - this will use the Groq API.  You can configure `--groq_api_key`, `--groq_model_name`, and `--groq_base_url`. To use it, set `--llm_service=marker.services.groq.GroqService`.

These services may have additional optional configuration as well - you can see it by viewing the classes.

## Groq LLM Integration

You can now use the Groq API (including the `compound-beta` model) as an LLM backend for Marker. To use it:

1. Install the `requests` library (already handled if you use Poetry).
2. Set your Groq API key in your environment:  
   ```zsh
   export GROQ_API_KEY=your_groq_api_key
   ```
3. Use the following CLI flag to select Groq as your LLM service:
   ```shell
   --llm_service=marker.services.groq.GroqService --groq_model_name=compound-beta
   ```
   You can also set `--groq_api_key` and `--groq_base_url` if needed.

Example:
```shell
marker_single input.pdf --use_llm --llm_service=marker.services.groq.GroqService --groq_model_name=compound-beta --groq_api_key=your_groq_api_key
```
...
# ... use as LLM service in your converter
```

See the `marker/services/groq.py` file for more details.
