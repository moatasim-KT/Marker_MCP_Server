...
class MetricsCollector:
        ...
        try:
            ...
            try:
                ...
                if cache_path.exists():
                    disk_usage_mb = sum(f.stat().st_size for f in cache_path.rglob('*') if f.is_file()) / (1024 * 1024)
            except Exception:
                pass
    ...
    async def _check_resource_alerts(self, metrics: ResourceMetrics):
        ...
        # Memory alerts
        if metrics.memory_percent > self.monitoring_config.alert_memory_threshold_percent:
        ...
        for job_id, job_metrics in self._active_jobs.items():
            ...
            if duration > self.monitoring_config.alert_processing_time_threshold_seconds:
    ...
    def start_operation(self, operation: str, file_path: Optional[str] = None) -> str:
        ...
        if len(self._active_jobs) >= self.resource_limits.max_concurrent_jobs:
            raise ValueError(f"Maximum concurrent jobs limit reached ({self.resource_limits.max_concurrent_jobs})")
        ...
        if file_path and os.path.exists(file_path):
            try:
                ...
                if file_size_mb > self.resource_limits.max_file_size_mb:
                    raise ValueError(f"File size {file_size_mb:.1f}MB exceeds limit of {self.resource_limits.max_file_size_mb}MB")
            except ValueError:
    ...
    def end_operation(self, job_id: str, success: bool = True, error_message: Optional[str] = None, 
                     pages_processed: Optional[int] = None):
        ...
        if HAS_PSUTIL and psutil is not None:
            try:
                ...
                metrics.cpu_time_seconds = sum(process.cpu_times()[:2])  # user + system time
            except Exception:
                pass
    ...
    def get_system_health(self) -> SystemHealth:
        ...
        with self._lock:
            recent_metrics = [m for m in self._metrics_history if current_time - m.timestamp < 300]  # Last 5 minutes
            recent_alerts = [a for a in self._alerts if current_time - time.time() < 3600]  # Last hour
        ...
        if recent_metrics:
            ...
            if latest.memory_percent > self.monitoring_config.alert_memory_threshold_percent:
    ...
    def get_metrics_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        with self._lock:
            recent_resource = [m for m in self._metrics_history if m.timestamp > cutoff_time]
            recent_performance = [m for m in self._performance_history if m.start_time > cutoff_time]
        
        summary = {
            "time_period_hours": hours,
            "resource_metrics": {
                "count": len(recent_resource),
                "memory_avg_mb": sum(m.memory_mb for m in recent_resource) / max(len(recent_resource), 1),
                "memory_max_mb": max((m.memory_mb for m in recent_resource), default=0),
                "cpu_avg_percent": sum(m.cpu_percent for m in recent_resource) / max(len(recent_resource), 1),
                "cpu_max_percent": max((m.cpu_percent for m in recent_resource), default=0),
            },
            "performance_metrics": {
                "total_operations": len(recent_performance),
                "successful_operations": sum(1 for m in recent_performance if m.success),
                "failed_operations": sum(1 for m in recent_performance if not m.success),
                "avg_duration_seconds": sum(m.duration_seconds or 0 for m in recent_performance) / max(len(recent_performance), 1),
                "max_duration_seconds": max((m.duration_seconds or 0 for m in recent_performance), default=0),
                "total_pages_processed": sum(m.pages_processed or 0 for m in recent_performance),
            },
    ...
    async def save_metrics_to_file(self, file_path: Optional[str] = None):
        ...
        if file_path is None:
            ...
            file_path = os.path.join(self.paths_config.metrics_dir, f"metrics_{timestamp}.json")
