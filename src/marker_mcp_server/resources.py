...
from .utils import get_logger, ResourceError
...
def _get_device_settings() -> Tuple[str, Optional[torch.dtype], Dict[str, Any]]:
    ...
    try:
        ...
        # Check for MPS (Apple Silicon)
            ...
            logger.warning("No GPU/accelerator found, using CPU. This will be slow for large documents.")
...
def _initialize_models() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            models = load_all_models(**device_kwargs)
            if not models:
...
def get_device() -> str:
    ...
    if not _initialized:
        try:
            ...
            logger.warning(f"Failed to initialize resources, falling back to CPU: {str(e)}")
            return "cpu"
...
def get_device_kwargs() -> Dict[str, Any]:
    ...
    if not _initialized:
        try:
            ...
            logger.warning(f"Failed to initialize resources, using default device kwargs: {str(e)}")
            return {}
...
def get_dtype() -> Optional[torch.dtype]:
    ...
    if not _initialized:
        try:
            ...
            logger.warning(f"Failed to initialize resources, using default dtype: {str(e)}")
            return None
