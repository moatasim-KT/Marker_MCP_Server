...
def normalize_llm_service(llm_service: str) -> str:
    ...
    logger.warning(f"Unknown LLM service '{llm_service}'. Use full module path or one of: {list(LLM_SERVICE_MAPPINGS.keys())}")
    return llm_service
...
try:
    ...
    if hasattr(marker_settings, 'TORCH_DEVICE'):
        marker_settings = type(marker_settings)(**{**marker_settings.model_dump(), 'TORCH_DEVICE': device})
        logger.info(f"Updated Marker TORCH_DEVICE to {device}")
        
    if hasattr(marker_settings, 'TORCH_DEVICE_MODEL'):
        marker_settings = type(marker_settings)(**{**marker_settings.model_dump(), 'TORCH_DEVICE_MODEL': device})
        logger.info(f"Updated Marker TORCH_DEVICE_MODEL to {device}")
...
async def _run_with_redirection(func, args=None, kwargs=None):
    ...
    try:
        ...
        # Run the function (await if it's a coroutine)
        ...
        error_msg = f"Error in _run_with_redirection: {str(e)}\n{traceback.format_exc()}"
...
async def handle_batch_convert(arguments: dict[str, Any]) -> dict:
    ...
    try:
        ...
        if not in_folder or not os.path.isdir(in_folder):
            raise ValueError(f"Invalid input directory: {in_folder}")
        ...
        if num_chunks > 1:
            pdf_files = [f for i, f in enumerate(pdf_files) if i % num_chunks == chunk_idx]

        # Support max_files
        if max_files is not None:
            pdf_files = pdf_files[:int(max_files)]
        ...
        for pdf_file in pdf_files:
            ...
            if skip_existing:
                config_parser = ConfigParser({**arguments, "file_path": input_path})
                out_folder = config_parser.get_output_folder(input_path)
                fname_base = config_parser.get_base_filename(input_path)
                output_ext = arguments.get("output_format", "markdown")
                ext = "md" if output_ext == "markdown" else output_ext
                output_file = os.path.join(out_folder, f"{fname_base}.{ext}")
...
async def handle_single_convert(arguments: dict[str, Any]) -> dict:
    """Convert a single file using Marker single conversion with full CLI argument support.
...
async def handle_chunk_convert(arguments: dict[str, Any]) -> dict:
    ...
    try:
        ...
        convert_args = {
            ...
            "disable_image_extraction": arguments.get("disable_image_extraction", False),
        ...
        for i in range(0, len(pdf_files), chunk_size):
            ...
            logger.info(f"Processing chunk {i//chunk_size + 1} with {len(chunk)} files")
...
async def handle_batch_pages_convert(arguments: dict[str, Any]) -> dict:
    ...
    try:
        ...
        try:
            ...
            pdf_doc.close()
            logger.info(f"Document has {total_pages} pages, processing {pages_per_chunk} pages per chunk")
        except Exception as e:
        ...
        for chunk_start in range(0, total_pages, pages_per_chunk):
            ...
            if not chunk_result.get("success"):
                logger.warning(f"Failed to process chunk {chunk_num} (pages {page_range}): {chunk_result.get('error')}")
            ...
            chunk_output_file = chunk_result.get("output_file")
            chunk_results.append({
                "chunk_num": chunk_num,
                "page_range": page_range,
                "success": True,
            ...
            if chunk_output_file and os.path.exists(chunk_output_file):
        ...
        if combine_output and chunk_files:
            ...
            combined_output_file = os.path.join(output_dir, f"{base_filename}_combined.{ext}")
            
            logger.info(f"Combining {len(chunk_files)} chunk outputs into {combined_output_file}")
                ...
                with open(combined_output_file, 'w', encoding='utf-8') as combined_file:
                    for i, chunk_file in enumerate(chunk_files):
                        ...
                        try:
                            ...
                            logger.warning(f"Failed to read chunk file {chunk_file}: {str(e)}")
                            combined_file.write(f"\n\n[Error reading chunk file: {chunk_file}]\n\n")
                
                logger.info(f"Successfully combined outputs into {combined_output_file}")
        ...
        if successful_chunks == total_chunks:
            result["message"] = f"Batch pages conversion completed successfully. Processed {total_pages} pages in {total_chunks} chunks."
        else:
            result["message"] = f"Batch pages conversion completed with {successful_chunks}/{total_chunks} chunks successful."
